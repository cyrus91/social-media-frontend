...
function ChatPage() {
  ...
  const markConversationRead = useMessagingStore(
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          const sent = await messagingService.sendVoiceMessage(
            conversationId,
            blob,
          );
          setMessages((prev) => [...prev, sent]);
          ...
          toast.error("Errore invio vocale");
        }
      };
      recorder.start();
  ...
  const handleAISummary = async () => {
    ...
    try {
      ...
      toast(res.data?.result || "Nessun riassunto disponibile", {
        duration: 6000,
      });
    } catch {
          ...
          {conversation && (
            <>
              ...
              <div>
                <button
                  onClick={() =>
                    navigate(`/profile/${conversation.otherUsername}`)
                  }
                  className="font-semibold text-gray-800 text-sm hover:text-blue-500 transition">
                  @{conversation.otherUsername}
                </button>
                <p
                  className={`text-xs ${otherOnline ? "text-green-500" : "text-gray-400"}`}>
                  {otherTyping ? (
            ...
            {showDisappearing && (
              <div className="absolute right-0 top-10 bg-white rounded-xl shadow-xl border border-gray-100 z-20 w-44 overflow-hidden">
                ...
                {[
                  ...
                  <button
                    key={label}
                    onClick={() => handleDisappearing(hours)}
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition ${
                      conversation?.disappearingHours === hours
                  ...
                  <div className="max-w-xs sm:max-w-sm lg:max-w-md space-y-0.5 min-w-0">
                    ...
                    {msg.replyToId && !deleted && (
                      <div
                        ...
                        <div
                          className={`px-3 py-1.5 rounded-xl text-xs border-l-2 border-blue-400 bg-gray-100 text-gray-500 max-w-full truncate`}>
                          <span className="font-semibold text-blue-500">
                            @{msg.replyToSenderUsername}
                          </span>
                          <span className="ml-1">
                            {msg.replyToContent || "Messaggio"}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="relative">
                      {deleted ? (
                        <div
                          className={`px-4 py-2 rounded-2xl text-sm italic text-gray-400 border border-dashed border-gray-300 ${mine ? "rounded-tr-sm" : "rounded-tl-sm"}`}>
                          🚫 Messaggio eliminato
                        </div>
                      ) : msg.audioUrl ? (
                        <div
                          className={`px-3 py-2 rounded-2xl ${mine ? "bg-blue-500 rounded-tr-sm" : "bg-white shadow-sm rounded-tl-sm"}`}>
                          <audio
                            controls
                            src={msg.audioUrl}
                      ...
                      {!deleted && (
                        <div
                          className={`absolute top-0 ${mine ? "left-0 -translate-x-full pr-2" : "right-0 translate-x-full pl-2"} hidden group-hover:flex items-center space-x-1`}>
                          {/* Reazione */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setReactionTarget(
                                reactionTarget === msg.id ? null : msg.id,
                              );
                            }}
                      ...
                      {reactionTarget === msg.id && (
                        <div
                          className={`absolute ${mine ? "right-0" : "left-0"} -top-10 bg-white rounded-full shadow-xl border border-gray-100 flex items-center px-2 py-1 space-x-1 z-20`}
                    ...
                    {/* Timestamp + spunte */}
                    <p
                      className={`text-xs px-1 text-gray-400 ${mine ? "text-right" : "text-left"}`}>
                      {msg.expiresAt && (
                        <span className="mr-1 text-orange-400">
                          ...
                          {Math.max(
                            0,
                            Math.round(
                              (new Date(msg.expiresAt) - new Date()) / 3600000,
                            ),
                          )}
                          h
                        </span>
