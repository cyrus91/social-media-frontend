...
const STORY_DURATION = 5000; // 5s per le immagini
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}
      ...
      <div style={{ position: "relative", width: "100%", height: "100%", maxWidth: "420px", margin: "0 auto", overflow: "hidden" }}>
        ...
        {/* Gradient bottom */}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "100px", background: "linear-gradient(to top, rgba(0,0,0,0.6), transparent)", pointerEvents: "none" }} />
        ...
        {/* Author header */}
        <div style={{ position: "absolute", top: "28px", left: "12px", right: "48px", display: "flex", alignItems: "center", gap: "10px", zIndex: 10 }}>
          <div onClick={() => { onClose(); navigate(`/profile/${currentGroup.authorUsername}`); }}
            ...
            {currentGroup.authorAvatarUrl ? (
              ...
              <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "var(--nx-grad-btn)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: "14px", border: "2px solid #fff" }}>
        ...
        {/* Close */}
        <button onClick={onClose} style={{ position: "absolute", top: "28px", right: "12px", zIndex: 10, background: "rgba(0,0,0,0.4)", border: "none", borderRadius: "50%", width: "32px", height: "32px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff" }}>
        ...
        {currentStory.caption && (
          <div style={{ position: "absolute", bottom: isOwn ? "60px" : "20px", left: "16px", right: "16px", zIndex: 10 }}>
        ...
        {/* Owner actions */}
        {isOwn && (
          <div style={{ position: "absolute", bottom: "16px", left: "16px", right: "16px", display: "flex", alignItems: "center", justifyContent: "space-between", zIndex: 10 }}>
            <button onClick={handleLoadViewers}
              style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "var(--nx-radius-full)", padding: "7px 14px", color: "#fff", fontSize: "12px", fontWeight: 600, cursor: "pointer" }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", zIndex: 5 }}>
          <div style={{ flex: 1, cursor: "pointer" }} onClick={goPrev} onMouseDown={() => setPaused(true)} onMouseUp={() => { setPaused(false); pausedAtRef.current = progress; }} />
          <div style={{ flex: 1, cursor: "pointer" }} onClick={goNext} onMouseDown={() => setPaused(true)} onMouseUp={() => { setPaused(false); pausedAtRef.current = progress; }} />
        </div>

        {/* Viewers panel */}
        {showViewers && (
          <div style={{ position: "absolute", inset: 0, zIndex: 20, background: "rgba(0,0,0,0.85)", display: "flex", flexDirection: "column" }}
            onClick={() => setShowViewers(false)}>
            <div style={{ marginTop: "auto", background: "var(--nx-surface)", borderRadius: "var(--nx-radius-xl) var(--nx-radius-xl) 0 0", padding: "20px", maxHeight: "60vh", overflowY: "auto" }}
              onClick={e => e.stopPropagation()}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
                <h3 style={{ fontWeight: 800, fontSize: "15px", color: "var(--nx-text)" }}>
                  Chi ha visto · {viewers.length}
                </h3>
                <button onClick={() => setShowViewers(false)}
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--nx-text-muted)", fontSize: "18px" }}>✕</button>
              </div>
              {loadingViewers ? (
                ...
                <div key={v.userId} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: "1px solid var(--nx-border)" }}>
                  {v.avatarUrl ? (
                    <img src={v.avatarUrl} alt={v.username} style={{ width: "36px", height: "36px", borderRadius: "50%", objectFit: "cover" }} />
                  ) : (
                    <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "var(--nx-grad-btn)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: "13px" }}>
