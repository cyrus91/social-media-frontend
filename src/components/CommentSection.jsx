  ...
  const handleReact = async (commentId, reaction) => {
    ...
    if (res.success) {
      setComments(prev => prev.map(c => {
        if (c.id === commentId) return { ...c, userReaction: res.data.userReaction, reactionCount: res.data.reactionCount };
        if (c.replies) {
          return { ...c, replies: c.replies.map(r => r.id === commentId ? { ...r, userReaction: res.data.userReaction, reactionCount: res.data.reactionCount } : r) };
        }
        return c;
      }));
    }
